...
function genUpdateBigData(tableName, updateArray, conditionArray) {
  const result = {
    sql : [],
    valueArr: []
  };
  const conditionArrayKeys = conditionArray.map((r)=>{return Object.keys(r)});
  const conditionArrayValues = conditionArray.map((r)=>{return Object.values(r)});
  
  if (updateArray.length != conditionArray.length) {
    return 'Invalid data provided!'
  }
  updateArray.forEach((obj,index)=>{
    let temp = genUpdate(tableName,obj);
    temp.sql += ` Where ${conditionArrayKeys[index][0]} = ?`;
    temp.valueArr.push(conditionArrayValues[index][0]);
    result.sql.push(temp.sql);
    result.valueArr.push(temp.valueArr);
  })
  return result;
}
...
module.exports = {
  ...
  genUpdateBigData:genUpdateBigData
}
